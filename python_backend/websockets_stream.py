...
def build_stream_base_url(handler: tornado.web.RequestHandler, port: int) -> str:
    public_host = get_public_host()
    if public_host.startswith("http://") or public_host.startswith("https://"):
        return f"{public_host.rstrip('/')}/video_feed/"
...
#     protocol = handler.request.protocol or "http"
#     return f"{protocol}://{local_ip}:{port}/video_feed/"
...
class AttendancePageHandler(BaseHandler):
    def get(self):
        ...
        self.render(
            ...
            streamBaseUrl=build_stream_base_url(self, self.application.listen_port),
        )
...
if __name__ == "__main__":
    ...
    print(f"🚀 Server bind: 0.0.0.0:{PORT}")
